...
if ($render_page) {
	...
	<div id="myaccount">
		<h1>Minha Conta</h1>
		<p>Bem-vindo à página da sua conta, <font color="blue"><strong><?php if ($config['ServerEngine'] !== 'OTHIRE') echo $user_data['name']; else echo $user_data['id']; ?></font></strong><br>
			<div class="inner">
		<?php if ($config['ServerEngine'] !== 'OTHIRE') {
			...
			if ($config['mailserver']['myaccount_verify_email']):
				...
				if ($user_znote_data['active_email'] == 1) {
					...
					?><br><strong>Seu e-mail não foi verificado! <a href="?authenticate">Por favor, verifique isso</a>.</strong><?php
				}
			endif; ?>
		</p>
